...
const Navbar = () => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div
          className={`fixed inset-0 bg-black opacity-50 z-40 ${darkMode ? 'bg-gray-900' : 'bg-gray-200'}`}
          onClick={() => setIsOpen(false)}
        ></div>
      )}

      {/* Barra de navegação principal */}
      <div className={`- z-50 navbar-bg shadow-navbar h-[123px] transition-transform duration-300 ease-in-out ${isOpen ? 'lg:hidden' : ''} ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
